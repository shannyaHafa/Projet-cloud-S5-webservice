...
public class ServiceAnnonceProduit {
    ...
    public HttpRetour historiqueAnnonce(String token) {
        HttpRetour resultat = new HttpRetour();
        DetailAnnonce[] listes = null;
        String idUtilisateur = null;
        try {
            ...
            if(verification==true) {
                idUtilisateur = new Token().readByToken(token).getUtilisateur();
                try {
                    listes = da.historiqueAnnonce(idUtilisateur);
                    resultat.setHttpRetour(resultat, 200, "Ok", listes);
                }
                catch(Exception exception) {
                    resultat.setHttpRetour(resultat, 400, exception.getMessage(), null);
    ...
    public HttpRetour getDetailAnnonce(String idAnnonce) {
        HttpRetour resultat = new HttpRetour();
        DetailAnnonce detailAnnonce = new DetailAnnonce();
        DetailAnnonce[] data = new DetailAnnonce[1];
        try {
            detailAnnonce = da.getById(idAnnonce);
    ...
    public HttpRetour listeAnnonceValiderUser(String token) {
        HttpRetour resultat = new HttpRetour();
        String idUtilisateur = null;
        DetailAnnonce[] listes = null;
        try {
            ...
            if(verification==true) {
                idUtilisateur = new Token().readByToken(token).getUtilisateur();
                try {
                    listes = da.getAnnonceUser("AnnonceValider", idUtilisateur);
                    resultat.setHttpRetour(resultat, 200, "ok", listes);
                }
                catch(Exception exception) {
                    resultat.setHttpRetour(resultat, 400, exception.getMessage(), null);
    ...
    public HttpRetour listeAnnonceNonValiderUser(String token) {
        HttpRetour resultat = new HttpRetour();
        String idUtilisateur = null;
        DetailAnnonce[] listes = null;
        try {
            ...
            if(verification==true) {
                idUtilisateur = new Token().readByToken(token).getUtilisateur();
                try {
                    listes = da.getAnnonceUser("AnnonceNonValider", idUtilisateur);
                    resultat.setHttpRetour(resultat, 200, "ok", listes);
                }
                catch(Exception exception) {
                    resultat.setHttpRetour(resultat, 200, exception.getMessage(), null);
                }
            }
            else {
                throw new Exception("Vous devez vous connectez!!");
